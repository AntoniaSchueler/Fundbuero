...
st.markdown(
    ...
    .fund-card { border:1.5px solid var(--green); background:white; padding:.65rem; height:100%; }
...
def predict_category(image):
    ...
    label = labels[index] if index < len(labels) else "Sonstiges"
...
def insert_item(conn, category, location, image_bytes):
    ...
    try:
        conn.execute(
            "INSERT INTO fundstuecke (id, category, location, image_filename, created_at, status) VALUES (?, ?, ?, ?, ?, ?)",
            (item_id, category, location.strip(), filename, utc_now().isoformat(), "verfuegbar"),
        )
        conn.commit()
    except Exception:
        image_path.unlink(missing_ok=True)
...
def get_available_items(conn, category=None):
    if category and category != "Alle":
        return conn.execute(
            "SELECT * FROM fundstuecke WHERE status='verfuegbar' AND category=? ORDER BY created_at DESC",
            (category,),
        ).fetchall()
    return conn.execute(
        "SELECT * FROM fundstuecke WHERE status='verfuegbar' ORDER BY created_at DESC"
    ).fetchall()


def mark_collected(conn, item_id):
    conn.execute(
        "UPDATE fundstuecke SET status='abgeholt', collected_at=? WHERE id=? AND status='verfuegbar'",
...
def admin_code():
    try:
        ...
        # Local fallback only. For a public deployment, set ADMIN_CODE in Streamlit Secrets.
...
st.markdown('<div class="brand-title">Fundbüro am<br> Katharineum</div>', unsafe_allow_html=True)
...
with nav2:
    if st.button("Gefunden", use_container_width=True):
        st.session_state.page = "gefunden"; st.rerun()
with nav3:
    if st.button("Verwaltung", use_container_width=True):
        st.session_state.page = "verwaltung"; st.rerun()
st.markdown("---")
...
def render_start():
    ...
    with left:
        st.markdown('<div class="hero-card"><h2>Verloren</h2><p>Du suchst einen Gegenstand? Durchsuche die aktuell gefundenen Sachen nach Kategorie.</p></div>', unsafe_allow_html=True)
        st.write("")
        if st.button("Gefundene Sachen ansehen", use_container_width=True):
            st.session_state.page = "gefunden"; st.rerun()
    with right:
        st.markdown('<div class="hero-card"><h2>Gefunden</h2><p>Du hast etwas gefunden? Lade ein Foto hoch. Die KI hilft bei der Einordnung.</p></div>', unsafe_allow_html=True)
        st.write("")
        if st.button("Fundstück melden", use_container_width=True):
            st.session_state.page = "upload"; st.rerun()
    st.markdown('<div class="footer">Ein verlorener Gegenstand findet vielleicht seinen Weg zurück.</div>', unsafe_allow_html=True)


def render_upload():
    st.header("Fundstück melden")
    st.write("Lade ein Foto hoch. Die KI schlägt eine Kategorie vor. Du kannst sie vor dem Speichern korrigieren.")
    uploaded = st.file_uploader("Foto auswählen", type=["jpg", "jpeg", "png"], help=f"Maximal {MAX_UPLOAD_MB} MB.")
    if uploaded is None:
        st.info("Bitte zuerst ein Foto auswählen."); return
    if uploaded.size > MAX_UPLOAD_MB * 1024 * 1024:
        st.error(f"Das Bild darf höchstens {MAX_UPLOAD_MB} MB groß sein."); return
    try:
        image, image_bytes = prepare_image(uploaded)
        predicted, confidence = predict_category(image)
    ...
    with right:
        ...
        category = st.selectbox("Kategorie bestätigen oder ändern", CATEGORIES, index=CATEGORIES.index(predicted))
        location = st.text_input("Fundort", placeholder="z. B. Sporthalle, Pausenhof, Raum 204 …", max_chars=200)
...
def render_found():
    ...
    if not items:
        st.info("Aktuell wurden keine passenden Fundstücke gefunden."); return
    ...
    for i, item in enumerate(items):
        with cols[i % 3]:
            ...
            st.markdown(f'<div class="fund-category">{item["category"]}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="fund-location">Fundort: {item["location"]}</div>', unsafe_allow_html=True)
            try:
...
def render_admin():
    ...
    st.write("Dieser Bereich ist für das Sekretariat. Hier können Fundstücke als abgeholt markiert werden.")
    code = st.text_input("Verwaltungscode", type="password")
    if not code:
        st.info("Bitte Verwaltungscode eingeben."); return
    if code != admin_code():
        st.error("Der Verwaltungscode ist nicht korrekt."); return
    st.success("Verwaltungsbereich geöffnet.")
    items = get_available_items(conn)
    if not items:
        st.info("Es sind aktuell keine offenen Fundstücke vorhanden."); return
    for item in items:
        with st.container(border=True):
            ...
            with right:
                confirm = st.checkbox("Als abgeholt markieren", key=f"confirm_{item['id']}")
                if confirm and st.button("Abholung speichern", key=f"collect_{item['id']}", use_container_width=True):
                    mark_collected(conn, item["id"])
                    st.success("Erledigt."); st.rerun()


if st.session_state.page == "start": render_start()
elif st.session_state.page == "gefunden": render_found()
elif st.session_state.page == "upload": render_upload()
elif st.session_state.page == "verwaltung": render_admin()
else:
    st.session_state.page = "start"; render_start()
